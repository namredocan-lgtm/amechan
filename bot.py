...
# Load Ame-chan prompt from file
...
with shelve.open('chatdata') as file:
	...
	for key in file.keys():
		if key.isnumeric():
			history = file[key]
			# Prepend the prompt if not present
			first_is_prompt = False
			if history and isinstance(history, list) and isinstance(history[0], dict):
				first = history[0]
				first_is_prompt = (first.get('role') == 'user' and bot_template[0]['parts'][0].strip() in first.get('parts', [''])[0])
			if not first_is_prompt:
				history = bot_template + history
			message_history[int(key)] = model.start_chat(history=history)
...
async def generate_response(channel_id,attachments,text):
	try:
		...
		# Ensure the prompt is always at the start
		first_is_prompt = False
		if message_history[channel_id].history and isinstance(message_history[channel_id].history, list) and isinstance(message_history[channel_id].history[0], dict):
			first = message_history[channel_id].history[0]
			first_is_prompt = (first.get('role') == 'user' and bot_template[0]['parts'][0].strip() in first.get('parts', [''])[0])
		if not first_is_prompt:
			message_history[channel_id].history = bot_template + message_history[channel_id].history
